...
logger=logging.getLogger("payment-service"); logger.setLevel(logging.INFO); logger.propagate=False
...
def log_event(message:str,*,level:str="INFO",**fields):
    payload={"@timestamp":datetime.now(timezone.utc).isoformat(),"service":{"name":"payment-service"},"log":{"level":level.lower()},"message":message,**fields}
